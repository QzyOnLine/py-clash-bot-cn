...
def start_button_event(
    ...
    if job_dictionary.get("emulator") == EmulatorType.ADB:
        ...
        if not device_serial or not AdbController.is_device_connected(device_serial):
            logger.change_status(f"启动已取消：ADB 设备 '{device_serial}' 未连接。")
            return None
    ...
    if job_dictionary.get("emulator") == EmulatorType.GOOGLE_PLAY:
        ...
        if device_serial and not validate_device_serial(device_serial):
            logger.change_status(f"启动已取消：设备序列号 '{device_serial}' 无效。")
            return None
...
class BotApplication:
    ...
    def _run_adb_command(self, serial: str, command: str):
        ...
        if not serial:
            self.logger.change_status("请先选择设备序列号。")
            return

        if not validate_device_serial(serial):
            self.logger.change_status(f"设备序列号格式无效：{serial}")
            return

        full_command = f"adb -s {serial} {command}"
        self.logger.change_status(f"正在运行 ADB 命令：{full_command}")
        try:
            ...
            if result.returncode == 0:
                self.logger.change_status(f"成功：{result.stdout.strip() if result.stdout else '(无输出)'}")
            else:
                self.logger.change_status(f"错误：{result.stderr.strip() if result.stderr else '(无错误信息)'}")
        except subprocess.TimeoutExpired:
            self.logger.change_status(f"ADB 命令超时：{full_command}")
        except FileNotFoundError:
            self.logger.change_status("未找到 ADB 命令。ADB 是否已安装并加入 PATH？")
        except Exception as e:
            self.logger.change_status(f"执行 ADB 命令失败：{e}")

    def _on_adb_refresh(self) -> None:
        self.logger.change_status("正在刷新 ADB 设备列表...")
        try:
            ...
            if devices:
                ...
                self.logger.change_status(f"找到设备：{', '.join(devices)}")
            else:
                self.ui.adb_serial_var.set("")
                self.logger.change_status("未找到 ADB 设备。")
        except Exception as e:
            self.logger.change_status(f"刷新 ADB 设备出错：{e}")
    ...
    def _refresh_device_dropdown(self, combo_widget, controller_class, emulator_name: str) -> None:
        ...
        try:
            ...
            if not devices:
                self.logger.change_status(f"未找到 {emulator_name} 的 ADB 设备")
        except Exception:
            logging.exception(f"Failed to refresh {emulator_name} device list")
            self.logger.change_status(f"无法发现 {emulator_name} 的设备")
    ...
    def _on_adb_connect(self) -> None:
        ...
        if not device_address:
            self.logger.change_status("设备地址/序列号不能为空。")
            return

        self.logger.change_status(f"正在尝试连接到 {device_address}...")
        self.ui.update_idletasks()

        try:
            if AdbController.connect_device(self.logger, device_address):
                self.logger.change_status(f"成功连接/验证 {device_address}！")
            ...
            self.logger.change_status(f"ADB 连接过程中出错：{e}")
    ...
    def _on_adb_set_size(self) -> None:
        ...
        self.logger.change_status(f"正在为 {serial} 设置尺寸为 {width}x{height}、密度为 {density}...")
    ...
    def _on_adb_reset_size(self) -> None:
        serial = self.ui.adb_serial_var.get()
        self.logger.change_status(f"正在重置 {serial} 的尺寸和密度...")
