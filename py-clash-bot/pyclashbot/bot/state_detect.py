...
def check_for_trophy_reward_menu(emulator) -> bool:
    ...
    # EN: the OK button text reads white at (200,603). On the CN (Tencent)
    # client the same glyph edge samples darker ([182,154,120] sampled live
    # 2026-09-04) while every other point matches, so the two sets are tried
    # independently.
    colors_en = [
        [255, 184, 68],
        [255, 175, 78],
    ...
    for colors in (colors_en, colors_cn):
        if all(pixel_is_equal(pixel, colors[i], tol=25) for i, pixel in enumerate(pixels)):
            return True
...
def check_pixels_for_win_in_battle_log(emulator) -> bool:
    ...
    """
    # CN (Tencent) client: each log entry prints 胜利 (win) in light blue or
    # 失败 (loss) in red at the top-left of the most recent entry. Scan the
    # text box for the light-blue glyphs; sampled off a live CN log at 419x633.
    iar = emulator.screenshot()
    win_text_hits = 0
    for y in range(140, 153, 2):
        for x in range(48, 79, 3):
            b, g, r = (int(v) for v in iar[y][x])
            if b > 190 and b - r > 80 and 120 < g < 220:
                win_text_hits += 1
                if win_text_hits >= 3:
                    return True
    # 胜利 glyphs absent -> 失败 (loss). (EN red-slash fallthrough below.)
    if win_text_hits:
        return True
...
def check_if_on_clash_main_burger_button_options_menu(emulator) -> bool:
    ...
    # CN (Tencent) client: the options menu lists 对战日志/排行榜/皇室频道/…
    # as orange buttons on a white panel; the EN text pixels above don't exist.
    # Vertical fingerprint along x=273 sampled off a live CN menu at 419x633.
    cn_pixels = [
        iar[20][273],  # white panel top edge
        iar[28][273],  # 对战日志 button top edge
        iar[52][273],  # 对战日志 button body
        iar[68][273],  # white gap below button
        iar[292][273],  # grey separator between button groups
        iar[388][273],  # 设置 button body
    ]
    cn_colors = [
        [255, 255, 255],
        [255, 187, 105],
    ...
    for i, color in enumerate(cn_colors):
        if not pixel_is_equal(cn_pixels[i], color, tol=25):
            return False
    return True
...
def check_if_on_card_page(emulator) -> bool:
    ...
    # colors5: CN (Tencent) client. Same layout as EN, but the selected tab shows
    # "卡组" instead of "Deck", so the two text-pixel landmarks (x=108 and x=146 on
    # the tab row) read as tab-body/glyph-edge colors instead of white letters.
    # Sampled off a live CN deck page at 419x633 (values are BGR, captured from the
    # emulator on 2026-09-04): the tab body is blue [209,68,41]-ish and the 卡组
    # glyph reads as a blue [204,65,41] + grey [100,69,64] pair instead of white.
    colors5 = [
        [221, 1, 235],
        [255, 255, 255],
        [209, 68, 41],
...
def check_if_on_battle_log_page(emulator) -> bool:
    ...
    # CN (Tencent) client: title reads 对战日志 (dark/gold glyphs), so the EN
    # white title-text points above never match. Fingerprint the shared layout
    # instead: red close X on blue tile + panel border colours. Sampled off a
    # live CN battle log page at 419x633.
    cn_pixels = [
        iar[64][366],  # close X top-left glow
        iar[79][365],  # close X blue tile below the cross
        iar[70][365],  # close X white cross centre
        iar[62][92],  # panel left border
        iar[77][316],  # panel lower border
        iar[108][105],  # 对战 tab (selected, light)
    ]
    cn_colors = [
        [147, 135, 254],
        [38, 38, 240],
    ...
    for i, p in enumerate(cn_pixels):
        if not pixel_is_equal(p, cn_colors[i], tol=25):
            return False
    return True
...
# CN (Tencent) war-page fingerprint, sampled 2026-09-04 from the live river-race
# view ("第1个对战日"). The EN palettes (pink/green/blue banner variants + bottom
# button bar) all miss this layout, so a CN-only set is added. Stable spots chosen
# by the user: the bright orange banner band across the top (y=12/16, clear of the
# day-title text) and the blue around the bottom "确定" button (its body at y~598
# and its lower edge at y~610, clear of the glyph). (x, y, r, g, b) in RGB.
_WAR_PAGE_CN_PIXELS: tuple[tuple[int, int, int, int, int], ...] = (
    (150, 12, 255, 185, 52),  # top orange banner, bright band
    (210, 12, 255, 185, 52),
    (270, 12, 255, 185, 52),
    (150, 16, 244, 174, 39),  # top orange banner, main band
    (270, 16, 244, 174, 39),
    (180, 598, 104, 187, 255),  # bottom 确定 button body
    (240, 598, 104, 187, 255),
    (175, 610, 78, 175, 255),  # bottom 确定 button lower edge
    (243, 610, 78, 175, 255),
)


def _matches_war_cn(iar) -> bool:
    """CN war-page fingerprint (see _WAR_PAGE_CN_PIXELS)."""
    for x, y, r, g, b in _WAR_PAGE_CN_PIXELS:
        bgr = iar[y][x]
        actual = [int(bgr[2]), int(bgr[1]), int(bgr[0])]
        if not pixel_is_equal(actual, [r, g, b], tol=25):
            return False
...
def check_if_on_war(emulator) -> bool:
    ...
    iar = emulator.screenshot()
    return (
        _matches_war_original(iar)
        or _matches_war_alt(iar)
        or _matches_war_alt2(iar)
        or _matches_war_cn(iar)
    )
...
# CN (Tencent) client full-screen event pages (e.g. "CRL 冠军之路" cheer event).
# These don't exist on the international client and block every other flow, so
# they get their own fingerprint + handler (nav.handle_cn_event_page). Pixels
# sampled BGR off a live CN client at 419x633: dark-blue patterned background,
# standard orange "返回" (return) button bottom-left, red banner mid-screen.
CN_EVENT_PAGE_PIXELS: list[tuple[tuple[int, int], list[int]]] = [
    ((115, 591), [255, 173, 79]),  # 返回 button left half
    ((135, 591), [255, 173, 79]),  # 返回 button mid
    ((50, 60), [176, 42, 10]),  # dark-blue bg top-left
    ((370, 60), [176, 42, 10]),  # dark-blue bg top-right
    ((209, 232), [135, 51, 13]),  # red banner center
    ((30, 300), [165, 70, 10]),  # dark-blue bg mid-left
]

CN_EVENT_PAGE_PIXEL_TOLERANCE = 30


def check_if_on_cn_event_page(emulator) -> bool:
    """True when a CN-client full-screen event page is covering the UI.

    Kept conservative (6 strict points) so normal menus never match; new CN
    event variants should extend this fingerprint list, not loosen it.
    """
    iar = emulator.screenshot()
    pixels = [iar[y][x] for (x, y), _ in CN_EVENT_PAGE_PIXELS]
    colors = [expected for _, expected in CN_EVENT_PAGE_PIXELS]
    return all_pixels_are_equal(pixels, colors, CN_EVENT_PAGE_PIXEL_TOLERANCE)
