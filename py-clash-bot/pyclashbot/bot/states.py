...
def any_battle_chain_enabled(job_list) -> bool:
    """True if any fight mode OR the clan-war job is enabled.

    The war state lives inside the same fight chain as 1v1/2v2/trophy, so the
    chain must be entered when war alone is toggled (otherwise the bot would
    idle in the clan_chat -> end_fight loop and never run a war battle).
    """
    return any_fight_mode_enabled(job_list) or job_list.get(
        UIField.WAR_USER_TOGGLE.value, False
    )
...
def state_tree(
    ...
    if state == "clan_chat":
        ...
        if clan_chat_state(
            ...
            if not any_battle_chain_enabled(job_list):
                logger.log("No fight modes enabled, skipping fight chain")
                return state_order.next_state("end_fight")
            return state_order.next_state(state)
