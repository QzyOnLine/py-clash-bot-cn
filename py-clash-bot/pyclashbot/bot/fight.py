...
def start_fight(emulator, logger, mode) -> bool:
    ...
    # Check if on clash main menu — wait for it instead of trusting a single
    # flaky check (a one-shot False right after a restart used to cascade into
    # a full app-restart loop on the CN client).
    logger.log("Checking if on main menu before starting fight...")
    if not wait_for_clash_main_menu(emulator, logger, timeout=20):
