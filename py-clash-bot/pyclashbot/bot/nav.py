...
from pyclashbot.bot.coords import (
    ...
    CN_EVENT_PAGE_RETURN_BUTTON_COORD,
    DECK_TABS_REGION,
...
def _find_trophy_collect_button(emulator):
    """Centroid of the largest green "收集" (collect) button on the trophy-road reward page, or None.

    The collect button's x/y vary with the current tier and scroll position, so it
    is located by its bright-green fill (connected-component analysis) instead of
    fixed coords.
    """
    import cv2
    import numpy as np
    ...
    return best if best_area >= 500 else None
...
def handle_trophy_reward_menu(
    ...
    # Click the green "收集" (collect) button first so the tier reward is actually
    # claimed (the CN client shows it on the current tier), then dismiss with OK.
    collect = _find_trophy_collect_button(emulator)
    if collect is not None:
        if printmode:
            logger.change_status(status="Collecting trophy road reward")
        else:
            logger.log("Clicking trophy road collect button")
        emulator.click(*collect)
        time.sleep(2)

    emulator.click(*OK_BUTTON_COORDS_IN_TROPHY_REWARD_PAGE)
    time.sleep(1)
...
def wait_for_clash_main_menu(
    ...
    while check_if_on_clash_main_menu(emulator) is not True:
        ...
        # handle CN-client full-screen event pages (e.g. "CRL 冠军之路")
        if check_if_on_cn_event_page(emulator):
            handle_cn_event_page(emulator, logger)
            time.sleep(2)
            continue
...
def select_mode(emulator, mode: str, logger: Logger | None = None):
    ...
    # must be on clash main. Don't trust a single check: right after an app
    # restart the CN client can still be settling (queued popups, animations)
    # and one flaky False used to cascade into a full app-restart loop.
    ...
    while time.time() - wait_start < 15:
        ...
        # Best-effort dismissal of things that can land on top of the menu.
        if check_if_on_cn_event_page(emulator):
            if logger is not None:
                handle_cn_event_page(emulator, logger)
            continue
    ...
    # Fallback: on the CN (Tencent) client there is no usable mode panel — the
    # main menu "对战" button IS Trophy Road (the default ladder), so restart-
    # looping here would never help. Treat Trophy Road as already selected.
    if mode == "Trophy Road":
        msg = "Mode panel search failed — falling back to default battle mode (Trophy Road)"
        if logger is not None:
            logger.change_status(msg)
        else:
            print(f"[!] {msg}")
...
def handle_cn_event_page(emulator, logger: Logger) -> bool:
    """Clear a CN-client full-screen event page (e.g. "CRL 冠军之路") by clicking
    its bottom-left "返回" (return) button.

    These pages don't exist on the international client; on the Tencent client
    they can cover the UI both at launch and right after a battle, which used to
    stall restart()/get_to_main_after_fight() until timeout.
    """
    logger.change_status("Handling CN event page popup...")
    emulator.click(*CN_EVENT_PAGE_RETURN_BUTTON_COORD)
    time.sleep(3)
    return True
...
def get_to_main_after_fight(emulator, logger):
    ...
    consecutive_main = 0
    SETTLE_S = 4  # CN clients land queued popups ~1-2s after main renders
    ...
    while time.time() - start_time < timeout:
        if check_if_on_clash_main_menu(emulator) is True:
            consecutive_main += 1
            if consecutive_main == 1:
                # First sighting: let queued CN popups land, then re-verify.
                time.sleep(SETTLE_S)
                continue
        ...
        # Main menu lost (e.g. a queued CN popup landed) — require a fresh
        # settle window before trusting main again.
        consecutive_main = 0
        ...
        # CN-client full-screen event pages (e.g. "CRL 冠军之路") can pop up
        # right after a battle; clear them before any other handling.
        if check_if_on_cn_event_page(emulator):
            handle_cn_event_page(emulator, logger)
            continue
