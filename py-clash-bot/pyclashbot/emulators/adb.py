...
from pyclashbot.bot.coords import CLAN_VOYAGE_CLOSE_BUTTON_COORDS, CLASH_MAIN_DEADSPACE_COORD, CN_EVENT_PAGE_RETURN_BUTTON_COORD
...
class AdbController(AdbBasedController):
    ...
    def restart(self) -> bool:
        ...
        # Prefer the global package, fall back to the Chinese (Tencent) release
        clash_pkg = CLASH_ROYALE_PACKAGE
        if not self.is_app_installed(clash_pkg) and self.is_app_installed(CLASH_ROYALE_PACKAGE_CN):
            self.logger.change_status("Detected Chinese (Tencent) Clash Royale package")
            clash_pkg = CLASH_ROYALE_PACKAGE_CN
        ...
        while time.time() < deadline:
            if check_if_on_clash_main_menu(self):
                ...
                if consecutive_main == 1:
                    # First sighting: give queued CN popups (e.g. the CRL event
                    # page) time to land, then re-verify from the loop top.
                    time.sleep(SETTLE_S)
                    continue
            ...
            # that block the main menu on the Tencent client at launch
            if check_if_on_cn_event_page(self):
                self.logger.change_status("Closing CN event page")
                self.click(*CN_EVENT_PAGE_RETURN_BUTTON_COORD)
                time.sleep(2)
                continue

            # Click in a safe area to dismiss potential pop-ups
            self.click(*CLASH_MAIN_DEADSPACE_COORD)
            time.sleep(2)
